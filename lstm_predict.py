...
def predict(begin, end, features, OHLC, predict_ticket, ratio_of_train, look_back, forecast_days, layers, learning_rate, epochs, batch_size):
    ...
    lstm_model = build_LSTM_Model(
        look_back, forecast_days, len(features), layers, learning_rate)
    lstm_model.summary()
    ...
    train_result = pd.DataFrame(
        data=d_train, index=df_train.index[-len(train_prediction):])
    ...
    test_result = pd.DataFrame(
        data=d_test, index=df_test.index[-len(test_prediction):])
